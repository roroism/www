...
<?
	...
	for ($i=0; $i<$count; $i++)
	{
		...
		$upfile_tmp_name[$i] = $files["tmp_name"][$i];	//임시파일.tmp 만들어졌다가 바로 사라진다.
		...
		//파일 이름 만들기 end
			if( $upfile_size[$i]  > 700000 ) {
				echo("
				<script>
				alert('업로드 파일 크기가 지정된 용량(700KB)을 초과합니다!<br>파일 크기를 체크해주세요! ');
				history.go(-1)
				</script>
				");
				exit;
			...
			if ( ($upfile_type[$i] != "image/gif") &&
				...
				($upfile_type[$i] != "image/pjpeg") && ($upfile_type[$i] != "image/png") && ($upfile_type[$i] != "image/x-png"))
			{
				echo("
					<script>
	...
	if ($mode=="modify")
	{
		...
		$sql = "update $table set subject='$subject', content='$content', recruitment_division='$recruitment_division', start_date='$start_date', end_date='$end_date' where num=$num";
		mysql_query($sql, $connect);  // $sql 에 저장된 명령 실행
	}
	else
	{
		...
		$sql = "insert into $table (id, name, nick, subject, content, regist_day, hit, is_html, recruitment_division, start_date, end_date,";
		$sql .= " file_name_0, file_name_1, file_name_2, file_copied_0,  file_copied_1, file_copied_2) ";
		$sql .= "values('$userid', '$username', '$usernick', '$subject', '$content', '$regist_day', 0, '$is_html', '$recruitment_division', '$start_date', '$end_date', ";
	...
	echo "
	   <script>
	    location.href = 'sub6_3.php?table=$table&page=$page';
	   </script>
	";
?>
